...
export function CheckoutForm({ locale }: { locale?: Locale }) {
  ...
  // GA4 whatsapp_click — final checkout conversion click. These step-4
  // buttons open wa.me via window.open (they are <button>s, not anchors), so
  // the delegated click listener in Layout.astro never sees them — push
  // explicitly, before opening the window. No double fire: delegation only
  // matches a[href^="https://wa.me/"] clicks. No-op unless PUBLIC_GTM_ID is
  // set at build time (pushEvent guard).
  const openWhatsAppConfirm = () => {
    if (!savedOrder) return;
    pushEvent(EVENT_NAMES.whatsapp_click, {
      whatsapp_phone: '573172137932',
      source: 'checkout',
    });
    window.open(
