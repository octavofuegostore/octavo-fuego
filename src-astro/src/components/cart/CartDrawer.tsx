        ...
        <div className="flex-1 overflow-y-auto p-6">
          {items.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-ceniza mb-4">{$t('cart.vacío')}</p>
        ...
        {items.length > 0 && (
          <div className="border-t border-humo/50 p-6 space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-ceniza">{$t('cart.subtotal')}</span>
              <span className="font-semibold text-lg">{formatCOP(total)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ceniza">{$t('cart.envio')}</span>
              <span className="text-success">{$t('cart.envioGratis')}</span>
            </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <button
            ...
            aria-label={$t('cart.ariaEliminar')}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
